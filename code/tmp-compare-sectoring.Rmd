...
disagree_any <- semi_join(master, dups, by = c("category", "sector")) %>%
    arrange(category, sector)
knitr::kable(disagree_any)
```


```{r}
# save relevant tables as Excel to share with Tom/Lisa
f <- "compare-sectoring.xlsx"
# xlsx_initialize_workbook(f)
xlsx_write_table(master, "master", f)
xlsx_write_table(disagree_share, "disagree_share", f)
xlsx_write_table(disagree_retail, "disagree_retail", f)
xlsx_write_table(disagree_group, "disagree_group", f)
xlsx_write_table(disagree_any, "disagree_any", f)
```
